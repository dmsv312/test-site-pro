...
backend/                Yii2 application
  ...
  commands/             console controllers (import/samples, import/file; clean/run; prepare/run; adgen/run; export/file, export/bulk)
  ...
  services/             import/ (readers, adapters, ImportService); cleaning/ (JunkRule, BrandRule, VolumeRule, CleaningService); preparation/ (AlreadyUsedRule, ForbiddenRule, ThemeClusterer, GroupingService, PreparationService); adgen/ (AdContent, RsaValidator, TemplateAdGenerator, StoredAdSource, AdGenerationService); export/ (CsvWriter — shared RFC-4180 renderer; GoogleAdsEditorExport — desktop combined CSV; GoogleAdsBulkUploadExport — web-UI per-entity sheets; ExportService)
  views/                site/ (login, error), import/ (dashboard, keywords), cleaning/ (funnel), prepare/ (funnel + campaign preview), ads/ (RSA preview), export/ (preview + both downloads), rules/ (thresholds + lists)
...
docker compose exec app php yii export/file      # write the Google Ads Editor (desktop) CSV (default: runtime/export/…)
docker compose exec app php yii export/bulk      # write the Google Ads web-UI bulk-upload ZIP (default: runtime/export/…)
