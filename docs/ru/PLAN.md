...
- **Генерация объявлений** — текст объявлений написан **заранее, локально, с помощью Claude
  Code CLI** и сохранён в JSON (по ключу `language:theme_key`); развёрнутый сервис предпочитает
  этот сохранённый текст, а при его отсутствии для группы откатывается на детерминированный
  шаблон по языку. На каждую группу — одно адаптивное поисковое объявление (на этом языке, с
  корректным целевым URL), валидируется как недоверенный ввод (ограничения длины заголовков/
  описаний, валидный UTF-8, обязательный URL). Развёрнутый хост ничего не генерирует, не делает
  платных API-запросов на каждый вызов и не хранит AI-ключей.
